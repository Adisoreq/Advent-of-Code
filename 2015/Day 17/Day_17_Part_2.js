...
const successful = Array.from(tl.successfulGenotypes)
let minCombinationSize = successful[0].size()

for (let i = 1; i < tl.successfulGenotypes.size; i++) {
    if (successful.size < minCombinationSize) {
        minCombinationSize = successful.size
    }
}

const result = successful.filter(genotype => genotype.size() === minCombinationSize).map(gt => gt.toString())
...
console.log(`Minimal matching combinations (size of ${minCombinationSize}): ${result.length}`)
